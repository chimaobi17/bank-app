...
export default function DashboardLayout({ children }: Props) {
    ...
    const navItems = [
        ...
        { name: 'Cards', icon: CreditCard, href: '/banking/accounts' },
        { name: 'Payments', icon: Send, href: '/banking/payments' },
        { name: 'Me', icon: User, href: '/profile' },
    ];

    return (
        <div className="flex min-h-[100dvh] flex-col items-center bg-black font-sans text-white selection:bg-primary/30">
            ...
            <div className="fixed bottom-0 z-50 flex w-full justify-center px-4 pb-6">
                <nav className="flex h-20 w-full max-w-[calc(28rem-2rem)] items-center justify-around rounded-[2.5rem] border border-white/5 bg-neutral-900/60 shadow-2xl backdrop-blur-2xl [-webkit-backdrop-filter:blur(40px)] transition-all duration-500">
                    {navItems.map((item) => {
