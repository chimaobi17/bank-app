...
export default function Landing() {
    ...
    return (
        <div className="min-h-screen bg-[#050505] text-slate-200 selection:bg-amber-500/30 selection:text-amber-200">
                    ...
                    {auth.user ? (
                        <div className="relative">
                            <button
                                ...
                                className="flex items-center gap-3 pl-4 pr-3 py-2 text-sm font-medium text-white bg-slate-900/50 border border-slate-800 rounded-2xl hover:border-amber-500/50 transition-all active:scale-95 group"
                            >
                                <span className="hidden sm:inline text-slate-400 group-hover:text-amber-400 transition-colors uppercase tracking-widest text-[10px] font-bold">Account</span>
                            ...
                            {isProfileOpen && (
                                <>
                                    ...
                                    <div className="absolute right-0 mt-3 w-56 p-1 bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl z-20 overflow-hidden animate-in fade-in zoom-in duration-200">
                                        <div className="px-4 py-3 border-b border-white/5">
                                            <p className="text-xs text-slate-500 uppercase font-black">Logged in as</p>
                                            <p className="text-sm font-bold text-white truncate">{auth.user.email}</p>
                                        </div>
                                        <Link
                                            href={dashboard()}
                                            className="flex items-center gap-3 px-4 py-3 text-sm text-slate-300 hover:bg-amber-500 hover:text-black transition-colors rounded-xl mt-1"
                                        >
                        ...
                        <>
                            ...
                            <Link
                                href={register().url}
                                className="px-6 py-2.5 text-sm font-black uppercase tracking-widest text-white border-2 border-amber-500/50 rounded-full hover:bg-amber-500 hover:text-black transition-all active:scale-95 shadow-[0_0_20px_rgba(245,158,11,0.2)]"
                            >
                                Sign Up
                            </Link>
                        </>
            ...
            <main className="relative z-10">
                <section className="px-6 pt-20 pb-32 mx-auto max-w-7xl lg:px-12 lg:pt-32">
                    <div className="max-w-4xl mx-auto text-center">
                        ...
                        <h1 className="text-5xl font-black tracking-tight text-white sm:text-7xl lg:text-8xl leading-tight">
                            The future of <br />
                        ...
                        <div className="flex flex-col items-center justify-center gap-4 mt-12 sm:flex-row">
                            <Link
                                href={register().url}
                                className="w-full sm:w-auto px-8 py-4 text-base font-bold text-black bg-amber-500 rounded-2xl hover:bg-amber-400 transition-all shadow-[0_10px_30px_rgba(245,158,11,0.3)] active:scale-95 flex items-center justify-center gap-2"
                            >
            ...
            <footer className="px-6 py-12 mt-24 border-t border-slate-900 lg:px-12">
...
function FeatureCard({ icon, title, description }: { icon: React.ReactNode, title: string, description: string }) {
    return (
        <div className="p-8 rounded-3xl border border-slate-900 bg-slate-900/20 backdrop-blur-sm hover:border-amber-500/30 hover:bg-slate-900/40 transition-all group">
            <div className="inline-flex items-center justify-center w-12 h-12 mb-6 rounded-2xl bg-slate-800 text-amber-500 group-hover:scale-110 transition-transform duration-500 shadow-inner">
                {icon}
            </div>
